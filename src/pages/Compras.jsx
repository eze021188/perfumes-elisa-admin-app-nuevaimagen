...
export default function Checkout() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      console.log('[Checkout] Productos cargados:', prodMapped); // Log productos cargados
    }
    loadData();
  }, []);
  ...
  const onAddToCart = producto => {
    console.log('[onAddToCart] Called with product:', producto); // Log when called and with which product
    setProductosVenta(prev => {
      console.log('[onAddToCart] Current productosVenta state:', prev); // Log current state
      const existe = prev.find(p => p.id === producto.id);

      if (existe) {
        console.log('[onAddToCart] Product already in cart:', existe); // Log if product exists
        // Verificar stock antes de aumentar la cantidad
        console.log(`[onAddToCart] Checking stock: ${existe.cantidad + 1} vs ${producto.stock}`); // Log stock check
        if (existe.cantidad + 1 > (producto.stock || 0)) {
          toast.error('Stock insuficiente');
          console.log('[onAddToCart] Stock insufficient. Returning previous state.'); // Log insufficient stock
        ...
        console.log('[onAddToCart] Product quantity increased. New state:', updatedCart); // Log updated state
        return updatedCart;
      }
      // Agregar nuevo producto con cantidad 1 y total parcial inicial
      const newCart = [...prev, { ...producto, cantidad: 1, total: producto.promocion ?? 0 }];
      console.log('[onAddToCart] Product added to cart. New state:', newCart); // Log new state
      return newCart;
    });
  };
