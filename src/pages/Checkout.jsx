...
export default function Checkout() {
  ...
  else if (discountType === 'Por porcentaje') subtotal = subtotal * (1 - discountValue / 100);
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      const num = (ventasPrevias?.length || 0) + 1;
      ...
      // Validar si falló la inserción principal
      if (errorVenta) {
        console.error('❌ Error al insertar en ventas:', errorVenta);
        toast.error('No se pudo registrar la venta principal.');
        setProcessing(false);
        return;
      }
      ...
      // Procesar detalle de venta, stock y movimientos
      const operaciones = productosVenta.map(async (p) => {
        ...
        if (errorDetalle) console.error(`❌ Error en detalle_venta (${p.nombre}):`, errorDetalle);
        ...
        const { error: errorStock } = await supabase
          ...
          .eq('id', p.id);
        if (errorStock) console.error(`❌ Error actualizando stock (${p.nombre}):`, errorStock);
        ...
        if (errorMovimiento) console.error(`❌ Error en movimiento_inventario (${p.nombre}):`, errorMovimiento);
      });
      ...
      console.error('❌ Error general al finalizar venta:', err);
  ...
  return (
    <div className="pt-4 pb-4 px-4 md:px-12">
      ...
      <ModalCheckout
        ...
        footer={
          <>
            <button onClick={() => setShowSaleModal(false)} className="px-4 py-2 bg-gray-200 rounded">Cancelar</button>
            <button onClick={handleFinalize} disabled={!paymentType || processing} className="px-4 py-2 bg-blue-600 text-white rounded disabled:opacity-50">Confirmar</button>
          </>
        }
      >
        ...
        <div className="mb-4">
          ...
          <div className="flex space-x-2">
            <select value={discountType} onChange={e => setDiscountType(e.target.value)} className="flex-1 border p-2 rounded">
              <option>Sin descuento</option>
              <option>Por importe</option>
              <option>Por porcentaje</option>
            </select>
            {(discountType === 'Por importe' || discountType === 'Por porcentaje') && (
              <input type="number" value={discountValue} onChange={e => setDiscountValue(Number(e.target.value))} className="w-24 border p-2 rounded" placeholder="Valor" />
            )}
          </div>
        </div>
      </ModalCheckout>
