...
const fetchPresupuestosExistentes = async () => {
  ...
  const handleAddItem = producto => {
    const newItem = {
      ...
      precio_unitario: producto.precio_promocion ?? producto.precio ?? 0 // Usar 0 como fallback final
    };
  ...
  const handleGuardarPresupuesto = async () => {
    ...
    const numero_presupuesto = `PRES-${año}-${String(Date.now()).slice(-6)}`; // Usar los últimos 6 dígitos del timestamp
      ...
      const ticketHtmlContent = `
        <div class="p-4 text-xs font-mono">
            <div class="text-center mb-4">
                ...
                <p class="mt-2 font-bold">Ticket #${presupuestoData.numero_presupuesto || 'N/A'}</p>
            </div>
            <div class="mb-4">
                <p>Cliente: ${presupuestoData.clientes?.nombre || 'N/A'}</p>
                <p>Vendedor: ${user?.email || 'N/A'}</p> {/* Usar email del user logueado */}
                <p>Fecha: ${formatReadableDate(presupuestoData.created_at || new Date())}</p> {/* Usar fecha del presupuesto o actual */}
            </div>
            <div class="border-t border-b border-gray-400 py-2 mb-4">
                <p class="font-bold mb-2">Detalle:</p>
                 ${presupuestoData.itemsPresupuesto ? // Si es presupuesto actual
                    presupuestoData.itemsPresupuesto.map(item => `
                        <p>${item.cantidad}x ${item.descripcion} - ${formatCurrency(item.precio_unitario)}</p>
                    `).join('')
                    : // Si es presupuesto histórico
                    presupuestoData.presupuesto_items.map(item => `
                         <p>${item.cantidad}x ${item.productos?.nombre || item.descripcion} - ${formatCurrency(item.precio_unitario)}</p>
                    `).join('')
                 }
            </div>
  ...
  const downloadTicketImage = () => {
      if (ticketPreviewData) {
          ...
          const filename = `ticket_${ticketPreviewData.numero_presupuesto || formatDateForFilename(new Date())}.jpg`;
                  ...
                  {filteredProductos.map((p, index) => (
                    <li
                      ...
                      onClick={() => {
                       ...
                       onMouseDown={(e) => e.preventDefault()}
                    >
                      {p.nombre} <span className="text-gray-500 text-xs">(${parseFloat(p.precio_promocion ?? p.precio ?? 0).toFixed(2)})</span> {/* Mostrar precio de promoción/normal */} {/* Corrected: Use curly braces for JSX expression */}
                    </li>
                  ))}
                </ul>
              )}
